...
### Рекомендация по размеру hero-фона
- Для desktop hero лучше использовать изображение не меньше `2560 × 1600 px`.
- Для mobile hero (`media.heroBgMobile`) — не меньше `1242 × 2208 px`.
- Такой запас по высоте нужен, чтобы фон уверенно закрывал hero и примыкающий верхний блок с иконками без чёрного прямоугольника на больших экранах.
